...
public class Tween extends EventDispatcher {
	...
	public void update() { // invoked once per frame by the TweenJuggler.
							// Updates this tween / DisplayObject
		if (this.gameClock == null) {
			gameClock = new GameClock();
		}

		if (!this.tweenOps.isEmpty()
				&& this.gameClock.getElapsedTime() >= Collections.max(this.tweenOps).getTweenTime()) {
			completedTween();
		}
		...
		for (TweenParam tweenOp : tweenOps) {
			if (tweenOp.getTweenTime() > this.gameClock.getElapsedTime()) {
				double percentDone = 1
						- ((tweenOp.getTweenTime() - this.gameClock.getElapsedTime()) / tweenOp.getTweenTime());
				double newValue = (tweenOp.getEndVal() - tweenOp.getStartVal())
						* TweenTransition.applyTransition(this.transition, percentDone) + tweenOp.getStartVal();
				this.setValue(tweenOp.getParamToTween(), newValue);
			}
		}
	}
